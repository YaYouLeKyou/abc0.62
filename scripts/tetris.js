...
function Tetris(options) {
  ...
  Tetramino.prototype.rotate = function () {
    ...
    for (var i = 0; i < temp.length; i++) {
      for (var j = 0; j < temp[i].length; j++) {
        if (
          fieldArray[this.coords[1] + i] !== undefined &&
  ...
  function drawNextTetramino() {
    ...
    for (var i = 0; i < nextTetramino.form.length; i++) {
      for (var j = 0; j < nextTetramino.form[i].length; j++) {
        predictionField.rows[i].cells[j].classList.add(
          nextTetramino.form[i][j]
        );
      }
    }
  }
  ...
  function moveTetramino() {
    controlTimeoutHandle = setTimeout(function () {
      if (currentTetramino.prevCoords) {
        for (var i = 0; i < currentTetramino.form.length; i++) {
          for (var j = 0; j < currentTetramino.form[i].length; j++) {
            ...
            fieldArray[i + currentTetramino.prevCoords[1]][
              j + currentTetramino.prevCoords[0]
            ] = '.';
          }
        }
      }
      ...
      for (var i = 0; i < currentTetramino.form.length; i++) {
        for (var j = 0; j < currentTetramino.form[i].length; j++) {
          if (currentTetramino.form[i][j] == '.') continue;
          fieldArray[i + currentTetramino.coords[1]][
            j + currentTetramino.coords[0]
          ] = currentTetramino.form[i][j];
        }
      }
  ...
  function checkBlocksUnderTetramino() {
    if (
      currentTetramino.coords[1] + currentTetramino.form.length ===
      fieldArray.length
    ) {
      changeTetramino();
      return;
    ...
    for (var i = currentTetramino.form.length - 1; i >= 0; i--) {
      for (var j = currentTetramino.form[i].length - 1; j >= 0; j--) {
        ...
        if (
          fieldArray[currentTetramino.coords[1] + i + 1][
            currentTetramino.coords[0] + j
          ] !== '.' &&
          fieldArray[currentTetramino.coords[1] + i][
            currentTetramino.coords[0] + j
          ] !== '.' &&
          (currentTetramino.form[i + 1] === undefined ||
            currentTetramino.form[i + 1][j] === '.')
        ) {
          changeTetramino();
          return;
        }
  ...
  function checkSideCoords(coords, side) {
    ...
    for (var i = 0; i < currentTetramino.form.length; i++) {
      if (
        fieldArray[currentTetramino.coords[1] + i][
          currentTetramino.coords[0] + side
        ] !== '.' &&
        currentTetramino.form[i][
          side === -1 ? 0 : currentTetramino.form[0].length - 1
        ] !== '.'
      ) {
        return false;
      }
